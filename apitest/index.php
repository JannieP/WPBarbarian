...
global $configdata;
$configdata ['consumer_key'] = "0f187127929a4432921e05f4c8069ada04e09d3dc";
$configdata ['consumer_secret'] = "76f52e735c729adce46afeb80356e36f";
function wpapi_oauth() {
	...
	switch ($state) { 
		default:
			...
			$callback = "http://mimeye.com/blog/wp-content/plugins/wp-barbarian/apitest";
			
			/* Build the authorization URL */
			$request_link = $to->getAuthorizeURL ( $token, $callback );
			
			/* Build link that gets user to flocks to authorize the app */
			$content = 'To Test c0nan click <a href="' . $request_link . '">HERE</a>';
...
if(!empty($_SESSION['oauth_access_token']) && !empty($_SESSION['oauth_access_token_secret'])) {
	...
	$xml_pure = $to->OAuthRequest ( 'http://c0nan.net/wpbarbarian/api/cs2ms.json', array (), 'POST' ) ;
	echo $xml_pure;
}
